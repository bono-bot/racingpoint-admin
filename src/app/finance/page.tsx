...
export default function FinancePage() {
  ...
  if (loading) return <div className="text-center text-rp-grey py-8">Loading...</div>;
    ...
    <div>
      ...
      {showBank && (
        <div className="bg-rp-card border border-rp-border rounded-xl p-5 mb-6">
          ...
          <p className="text-sm text-neutral-400 mb-4">Upload a CSV or image of your bank statement. AI will parse transactions and match them to existing purchases/sales.</p>
          ...
          {parsing && <div className="text-sm text-neutral-400 py-4 text-center">Parsing statement...</div>}

          {transactions.length > 0 && (
            <div>
              <div className="flex justify-between items-center mb-3">
                <p className="text-sm text-neutral-400">{transactions.length} transactions found, {transactions.filter(t => t.match).length} matched</p>
                <button onClick={saveTransactions} disabled={saving}
                  className="px-4 py-2 bg-green-600 hover:bg-green-500 disabled:bg-rp-card rounded-lg text-sm font-medium">
                  {saving ? 'Saving...' : 'Save All'}
                </button>
              </div>
              <div className="border border-rp-border rounded-lg overflow-hidden max-h-80 overflow-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-rp-card">
                  ...
                  <tbody>
                    {transactions.map((tx, i) => (
                      <tr key={i} className="border-b border-rp-border/50">
                        <td className="px-3 py-2 text-neutral-400">{formatDate(tx.date)}</td>
                        <td className="px-3 py-2 max-w-[200px] truncate">{tx.description}</td>
                        <td className="px-3 py-2">
                          <span className={`text-xs px-2 py-0.5 rounded-full ${
                            tx.type === 'credit' ? 'bg-green-500/10 text-green-400' : 'bg-rp-red/10 text-rp-red'
                          }`}>{tx.type}</span>
                        </td>
                        ...
                        <td className="px-3 py-2">
                          {tx.match ? (
                            ...
                            <span className="text-xs text-rp-grey">Unmatched</span>
                          )}
                        </td>
                      </tr>
                    ))}
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-rp-card border border-rp-border rounded-xl p-5">
          ...
          {byPayment.length === 0 ? (
            <p className="text-sm text-rp-grey">No sales data yet</p>
          ) : (
              ...
              {byPayment.map(p => (
                <div key={p.payment_method} className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-neutral-400">{p.count} transactions</span>
                  </div>
        ...
        <div className="bg-rp-card border border-rp-border rounded-xl p-5">
          ...
          {purchasesByCategory.length === 0 ? (
            <p className="text-sm text-rp-grey">No expense data yet</p>
          ) : (
            <div className="space-y-3">
              {purchasesByCategory.map(p => (
                <div key={p.category} className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-neutral-400">{p.count} purchases</span>
                  </div>
                  <span className="font-mono text-sm">{fmt(p.total)}</span>
                </div>
              ))}
        ...
        <div className="bg-rp-card border border-rp-border rounded-xl p-5 col-span-2">
          ...
          {topItems.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {topItems.map((item, i) => (
                <div key={item.item_name} className="flex justify-between items-center p-3 bg-rp-card/30 rounded-lg">
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-rp-grey w-5">#{i + 1}</span>
