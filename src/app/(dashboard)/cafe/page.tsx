    ...
    <div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        <button onClick={() => setFilter('all')}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium ${filter === 'all' ? 'bg-rp-red/10 text-rp-red border border-rp-red/20' : 'bg-rp-card border border-rp-border text-neutral-400'}`}>
          All ({items.length})
        </button>
        {categories.map(c => (
          <button key={c} onClick={() => setFilter(c)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium ${filter === c ? 'bg-rp-red/10 text-rp-red border border-rp-red/20' : 'bg-rp-card border border-rp-border text-neutral-400'}`}>
            {c}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {Object.entries(grouped).map(([cat, catItems]) => (
            <div key={cat}>
              <h2 className="text-lg font-semibold mb-3 text-neutral-300">{cat}</h2>
              <div className="bg-rp-card border border-rp-border rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {catItems.map(item => (
                      <tr key={item.id} className="border-b border-rp-border/50 hover:bg-rp-card/30">
                        <td className="px-4 py-3 font-medium">{item.name}</td>
                        <td className="px-4 py-3">
                          <span className={`text-xs px-2 py-0.5 rounded-full ${item.veg ? 'bg-green-500/10 text-green-400' : 'bg-rp-red/10 text-rp-red'}`}>
                            {item.veg ? 'Veg' : 'Non-Veg'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right font-mono text-neutral-300">₹{item.price}</td>
                        <td className="px-4 py-3 text-right">
                          <button onClick={() => toggleAvailable(item.id, item.available)}
                            className={`text-xs px-3 py-1 rounded-full ${item.available ? 'bg-green-500/10 text-green-400 hover:bg-green-500/20' : 'bg-rp-card text-rp-grey hover:bg-rp-card'}`}>
