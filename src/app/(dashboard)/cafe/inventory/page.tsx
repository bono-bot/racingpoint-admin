...
export default function InventoryPage() {
  ...
  return (
    <div>
      ...
      {showAdd && (
        <div className="bg-rp-card border border-rp-border rounded-xl p-5 mb-6 grid grid-cols-3 gap-4">
      ...
      {loading ? (
        ...
        <div className="bg-rp-card border border-rp-border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map(item => (
                <tr key={item.id} className={`border-b border-rp-border/50 ${item.low_stock ? 'bg-amber-500/5' : 'hover:bg-rp-card/30'}`}>
                  ...
                  <td className={`px-4 py-3 text-right font-mono ${item.low_stock ? 'text-amber-400' : 'text-neutral-300'}`}>
                    {item.quantity} {item.unit}
                  </td>
                  <td className="px-4 py-3 text-right text-rp-grey">{item.min_stock} {item.unit}</td>
                  <td className="px-4 py-3 text-right font-mono text-neutral-400">₹{item.cost_per_unit}</td>
                  <td className="px-4 py-3 text-right">
                    {adjusting === item.id ? (
                      <div className="flex gap-2 items-center justify-end">
                        <select value={adjustment.type} onChange={e => setAdjustment({...adjustment, type: e.target.value as 'in' | 'out'})}
                          className="bg-rp-card border border-rp-border rounded px-2 py-1 text-xs">
                          <option value="in">Stock In</option>
                          <option value="out">Stock Out</option>
                        </select>
                        <input type="number" placeholder="Qty" value={adjustment.quantity}
                          onChange={e => setAdjustment({...adjustment, quantity: e.target.value})}
                          className="bg-rp-card border border-rp-border rounded px-2 py-1 text-xs w-16" />
                        ...
                        <button onClick={() => setAdjusting(null)}
                          className="px-2 py-1 bg-rp-card rounded text-xs">X</button>
                      </div>
                    ) : (
                      <button onClick={() => setAdjusting(item.id)}
                        className="text-xs text-neutral-400 hover:text-white">Adjust</button>
                    )}
                  </td>
                </tr>
              ))}
